...
public class ShardListScreen extends Screen {
	...
	private static final int VIEW_HUB = 0;
	private static final int VIEW_SKILL = 1;
	private static final int VIEW_FLAT = 2;
	...
	protected void init() {
		...
		java.util.Map<String, int[]> prog = new java.util.HashMap<>();
		for (String k : ShardDb.allShards()) {
			ShardDb.Shard s = ShardDb.shard(k);
			...
			int lvl = ShardProgress.displayLevel(k);
			if (lvl > 0) p[0] += lvl;
		}
	...
	private void drawFamilyList(GuiGraphicsExtractor ctx, int mouseX, int mouseY) {
		...
		int maxRows = Math.min(rows, (this.height - y - 8) / 13);
	...
	private void drawRecent(GuiGraphicsExtractor ctx, int mouseX, int mouseY) {
		...
		for (int i = 0; i < keys.size(); i++) {
			...
			ctx.fill(cx, cy, cx + 2, cy + 16, rarityColor(s.rarity));
	...
	public boolean mouseClicked(MouseButtonEvent event, boolean doubled) {
		...
		if (in(mx, my, toggleX, 16, toggleX + toggleW, 30)) {
			...
			if (viewMode == VIEW_SKILL) sourceFilter = null;
